...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof iterNonSquaresSeq, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object which generates a sequence of nonsquares', function test( t ) {
	var expected;
	var actual;
	var it;
	var i;
	...
	for ( i = 0; i < 1000; i++ ) {
		...
		t.strictEqual( isSquareNumber( actual.value ), false, 'does not return a square number' );
...
tape( 'if an environment supports `Symbol.iterator`, the returned iterator is iterable', function test( t ) {
	var iterNonSquaresSeq;
	var it1;
	var it2;
	var i;

	iterNonSquaresSeq = proxyquire( './../lib/main.js', {
		'@stdlib/symbol-iterator': '__ITERATOR_SYMBOL__'
	});

	it1 = iterNonSquaresSeq();
